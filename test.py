...
ENABLEX_APP_ID = os.getenv("ENABLEX_APP_ID", "69142da3bb3a72e245017493")
ENABLEX_APP_KEY = os.getenv("ENABLEX_APP_KEY", "3a8ugePebujasu2y3yjaHu7ygyaamuZunavy")
...
print(f"App Key: {ENABLEX_APP_KEY[:10]}...")
...
async def voice_webhook(request):
    ...
    called_number = data.get('to', data.get('called_number', 'Unknown'))
    event = data.get('event', 'call.initiated')
    
    print(f"\nEnableX Event: {event}")
    print(f"Call from {caller_number} to {called_number} (Call ID: {call_id})")
    
    # EnableX uses JSON responses
    enablex_response = {
        "action": [
            {
                "type": "play",
            ...
            },
            {
                ...
                "trim_silence": True,
                "format": "wav",
                "recording_status_callback": "https://calling-test-2kdd.onrender.com/recording"
            },
            {
                "type": "play",
...
async def process_recording(recording_url, call_sid):
    ...
    try:
        ...
        auth = BasicAuth(ENABLEX_APP_ID, ENABLEX_APP_KEY)
